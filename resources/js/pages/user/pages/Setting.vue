<template>
    <div :class="$route.meta.wide ? 'md:w-3/6' : 'md:w-2/6'" class="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 w-full h-full md:h-[calc(100vh-114px)] p-0 md:p-4 z-10 bg-slate-100 md:bg-transparent dark:bg-slate-700">
        <div class="block space-y-3 md:space-y-4">
            <div class="grid grid-cols-12 gap-x-3 gap-y-4 relative w-full p-4 rounded-none md:rounded-lg shadow-none md:shadow-md bg-white dark:bg-slate-800">
                <div class="col-span-12 flex justify-center items-center space-x-3">
                    <div class="relative">
                        <img :src="store.getters['auth/user'].avatar" referrerpolicy="no-referrer" alt="Foto Profil" class="w-12 aspect-square rounded-full">
            ...
            <Form @submit="updateProfile" :validation-schema="schemaProfile" v-slot="{ errors }" class="grid grid-cols-12 gap-x-3 gap-y-4 relative w-full p-4 rounded-none md:rounded-lg shadow-none md:shadow-md bg-white dark:bg-slate-800">
        ...
        <div class="block space-y-3 md:space-y-4">
            <Form @submit="changePassword" :validation-schema="schemaSecurity" v-slot="{ errors }" class="grid grid-cols-12 gap-x-3 gap-y-4 relative w-full p-4 rounded-none md:rounded-lg shadow-none md:shadow-md bg-white dark:bg-slate-800">
                <div class="col-span-12">
    ...
    export default {
        ...
        setup() {
            ...
            const updateProfile = async (values) => {
                ...
                axios.patch('/api/user/profile', values).then(({ data }) => {
            ...
            const changePassword = async (values) => {
                ...
                axios.patch('/api/user/password', values).then(({ data }) => {
